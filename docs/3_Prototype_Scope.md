...
| **Planner**              | Token counting (OpenAI `tiktoken`, Gemini `countTokens`); dual guards (85 % in / 95 % out); array streaming planner; CSV row sizing;                                                                                | —                        | Dynamic model-tier switch (Flash → Pro)           | Planner owns all token maths; extractor never worries about limits.                                                                                                                                    |
